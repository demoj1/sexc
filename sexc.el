...
(defcustom sexc/surface-keywords
  '("include" "define" "defn" "decl" "block" "if" "cond"
    ...
    "." "->" "not" "+" "-" "*" "/" "%" "=" "not=" "<" "<=" ">"
    ">=" "&&" "and" "||" "or" "post-inc" "nop"
    "nil" "do" "nil?" "not-nil?" "zero?" "nonzero?" "ltz?" "letz?" "gtz?" "getz?"
    "pos?" "neg?" "even?" "odd?" "bit-set?" "between?"
...
(defcustom sexc/eldoc-docs
  '(("defn" . "(defn RET NAME PARAMS FORM...) -> define function")
    ...
    ("defsum" . "(defsum Name :variants (Variant (TYPE FIELD)...) ... [:methods (defn ...)...]) -> tagged union; construct Name/Variant#, branch with match")
