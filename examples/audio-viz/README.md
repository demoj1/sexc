...
- В `%module ring` нельзя иметь struct-поле и module-функцию с одним именем
  (поле `cap` → `capacity` для обхода). `%module` рерайтит идентификаторы
  без учёта контекста `->`/`.`.
- `%import` дедуплицирован по DFS-веткам, но не глобально: один файл,
  импортируемый из двух мест, загружается дважды (поэтому здесь `graph.sexc`
  не импортирует `ring-buffer.sexc` напрямую — это делает `main.sexc`).
